...
QMem 是一个面向 AI Agent 时代的个人记忆迁移与持续维护工具。它把用户散落在不同 AI 平台、不同对话和不同项目里的长期信息整理成一个本地、可读、可导出、可注入的标准化知识库，让用户不必每换一个模型或平台就重新介绍自己、自己的项目和自己的工作方式。
...
因此，QMem 的知识库更接近一个“本地个人记忆 wiki”：Markdown 面向人类阅读，JSON 面向程序调用，导出包面向跨平台迁移。
...
用户在一个源平台上积累了大量历史对话和平台记忆，希望迁移到另一个目标平台。QMem 会：
